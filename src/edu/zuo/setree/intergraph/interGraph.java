...
class pair extends Object{
    ...
    public void setRight(long right){ this.right = right; }
...
public class interGraph {
        ...
        try {
            ...
            while(consEdgeGraphInput.hasNextLine()) {
                ...
                if (line.length() == 0) {
                    ...
                    funcParamReturn.put(funcIndex,new LinkedHashMap<String, Long>());
                } else {
                    String[] tokens = line.split(", ");
                    if (tokens.length == 2) {
                ...
                if(line.length()==0){
                    ...
                    if(tokens.length==2){
                        ...
                        */
                        ...
                        long n2_right = Long.parseLong(tokens[3].substring(0,tokens[3].length()-1));
                        ...
                        if(tokens[4].length()>2){
                            long first_right = Long.parseLong(tokens[4].substring(1,tokens[4].length()-1));
                            if(!(funcParamReturn.containsKey(callFuncIndex)&&funcParamReturn.get(callFuncIndex).containsKey("[Callee]")))continue;
                            long second_right = funcParamReturn.get(callFuncIndex).get("[Callee]");
                            pair p1 = new pair(funcIndex, first_right);
                        ...
                        if(paraN == 1){
                            ...
                            long first_right = Long.parseLong(tokens[5].substring(1,tokens[5].length()-1));
                            if(!(funcParamReturn.containsKey(callFuncIndex)&&funcParamReturn.get(callFuncIndex).containsKey("[Para0]")))continue;
                            long second_right = funcParamReturn.get(callFuncIndex).get("[Para0]");
                            pair p1 = new pair(funcIndex, first_right);
                            pair p2 = new pair(callFuncIndex, second_right);
                            long i1 = pair2indexMap.get(p1);
                            long i2 = pair2indexMap.get(p2);
                            ...
                            long first_right = Long.parseLong(tokens[5].substring(1,tokens[5].length()));
                            if(!(funcParamReturn.containsKey(callFuncIndex)&&funcParamReturn.get(callFuncIndex).containsKey("[Para0]")))continue;
                            long second_right = funcParamReturn.get(callFuncIndex).get("[Para0]");
                            pair p1 = new pair(funcIndex, first_right);
                            pair p2 = new pair(callFuncIndex, second_right);
                            ...
                            for(int i = 1;i<paraN-1;i++){
                                first_right = Long.parseLong(tokens[5+i].substring(0,tokens[5+i].length()));
                            ...
                            first_right = Long.parseLong(tokens[5+paraIndex].substring(0,tokens[5+paraIndex].length()-1));
                        ...
                        if(tokens[0].length()>2){
                            long second_right = Long.parseLong(tokens[0].substring(1,tokens[0].length()-1));
                            pair p2 = new pair(funcIndex, second_right);
                            long i2= pair2indexMap.get(p2);
                            pair p4 = new pair(funcIndex, funcNodeIndex);
                            Map<String, Long> t = funcParamReturn.get(callFuncIndex);
                            for(String s:t.keySet()){
                                if(s.startsWith("[Return")){
                                    long first_right = t.get(s);
                                    long callFuncRetNode = Long.parseLong(s.substring(7,s.length()-1));
    ...
    public static String add_ite(String line){
        ...
        if(i2!=-1&&i2<split1)split1=i2+5;
        int i3=line.indexOf("(bvxor ");
        if(i3!=-1&&i3<split1)split1=i3+6;
        ...
        String p4 = "";
        if(split3+1<line.length())p4 = " "+add_ite(line.substring(split3+1).trim());
        return p1+" "+p2+" "+p3+")"+p4;
    }
    ...
    public static void printSmt2() {
        try{
            ...
            while(conditionalSmt2Input.hasNextLine()) {
                ...
                if (line.contains("ishr") || line.contains("ishl") || line.contains("iushr") || line.contains("lshr") || line.contains("lshl") || line.contains("lushr"))
                    continue;
                if (line.contains("(iand ")||line.contains("ior ")||line.contains("ixor ")||line.contains("(land ")||line.contains("lor ")||line.contains("lxor ")) {
                    line = line.replace("(iand ", "(bvand ").replace("(ior ", "(bvor ").replace("(ixor ", "(bvxor ").replace("(land ", "(bvand ").replace("(lor ", "(bvor ").replace("(lxor ", "(bvxor ");
                    line = add_ite(line);
                }
                if(line.length()==0){
        ...
        try {
            ...
            while (consEdgeGraphInput.hasNextLine()) {
                ...
                if(line.length()==0){
                    ...
                    String[] tokens = line.split(", ");
                    if (tokens.length > 4) {
                        long start = line.indexOf("<");
                        long end = line.indexOf(">")+1;
            ...
            for(String key:result.keySet()){
                ...
                if(size!=0) {
                    ...
                    callGraphOut.print(keySplit[1]+" "+keySplit[2].substring(0,keySplit[2].lastIndexOf(">"))+" "+keySplit[0].substring(1,keySplit[0].indexOf(":")) + "\t" + size);
                    for (String val:callMap.get(key)){
                        String valSplit[] = val.split(" ");
                        callGraphOut.print("\t"+valSplit[1]+" "+valSplit[2].substring(0,valSplit[2].lastIndexOf(">"))+" "+valSplit[0].substring(1,valSplit[0].indexOf(":")));
                    }
